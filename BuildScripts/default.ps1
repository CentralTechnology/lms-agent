	...
	try {
		...
		exec {
			if(isAppVeyor) {
				...
				& $gitVersionExe /l console /output buildserver /UpdateAssemblyInfo /b $env:APPVEYOR_REPO_BRANCH
...
Task -Name BuildSolution -Depends __RemoveBuildArtifactsDirectory, __VerifyConfiguration, __InstallPSBuild, __EchoAppVeyorEnvironmentVariables, RunGitVersion, NugetPackageRestore -Description "Builds the main solution for the package" -Action {
	...
	try {
		...
		exec {
			Invoke-MSBuild "$sourceDirectory\LicenseMonitoringSystem.sln" -NoLogo -Configuration $config -Platform $platform -Targets Build -DetailedSummary ;
